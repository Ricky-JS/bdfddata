module.exports = async (req, res, Discord, fetch, config, t) => {
        ...
        let slid = req.headers['slash']
        if(!aid) return res.send({status:400, error: `${config.errors.headers.appId}`, api: Object.assign(config.info, {ping : `${(Date.now() - t)}ms`})})
let re = await fetch(`https://discord.com/api/v10/applications/${aid}/commands`, {
method: 'GET',
headers: {
