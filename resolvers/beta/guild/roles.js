    ...
    if(type === 'default') {
        if (Array.isArray(re)) {
            re?.forEach(r => {
                ...
                if (re[re.indexOf(r)]?.icon) re[re.indexOf(r)].icon = re[re.indexOf(r)].icon ? 'https://cdn.discordapp.com/role-icons/' + re[re.indexOf(r)].id + '/' + re[re.indexOf(r)].icon + '.png' : null || null
            })
        } else {
            if (re?.permissions) re.permissions = new Discord.PermissionsBitField(re.permissions)?.toArray()
            if (re?.icon) re.icon = re.icon ? 'https://cdn.discordapp.com/role-icons/' + re.id + '/' + re.icon + '.png' : null || null
        }
        return re;
}
