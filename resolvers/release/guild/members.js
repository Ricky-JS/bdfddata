...
module.exports = async (re, type = 'default') => {
    ...
    if (Array.isArray(re)) {
        re?.forEach(r => {
            ...
            if (r?.avatar) re.avatar = r?.avatar ? 'https://cdn.discordapp.com/avatars/' + r?.user?.id + '/' + r?.avatar + (r?.avatar?.startsWith(`a_`) ? '.gif' : '.png') : null || null
        ...
        if (re?.avatar) re.avatar = re?.avatar ? 'https://cdn.discordapp.com/avatars/' + re?.user?.id + '/' + re?.avatar + (re?.avatar?.startsWith(`a_`) ? '.gif' : '.png') : null || null
